...
class App extends React.Component {
  ...
  get allTimes() {
    const times = _.map(this.state.prayers, function(prayer) {
      return prayer.time;
    });
    return times;
  }
    ...
    return (
      <div className="App">
        <div className="header">
          <div className="date">
            <Clock date={Date.now()} format={"ddd YYYY MM"} />
            <br />
          </div>
...
const Card = props => {
  return (
    <Row className={`prayer-card-item ${props.name} `}>
      <Col className="name">
      ...
      <Col className="time">
        <TimeConversion time={props.time} />{" "}
      </Col>
